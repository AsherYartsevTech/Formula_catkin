...
public:
    ...
    // C'tor
    PurePursuitController(double ld, double velocity, vector<pair<double,double>> waypoints);
    ...
    */
   double control_step(const vector<double>& Pose, double& velocity,
                       double& alpha)
   {
        double lat_error;
        // currently the velocity is constant
                       ...
                       pair<double,double>& ld_point)
    {
        ...
        if (slope_is_inf) {
            ...
            a = 1;
            b = (-2) * car.second;
            c = (pow(car.first-x0, 2) + pow(car.second,2) - pow(this->ld,2));
        } else {
            y0 = last_wp.second - (m * last_wp.first);
            a = 1 + pow(m,2);
            b = (-2) * car.first + 2 * m * (y0-car.second);
            c = (pow(car.first,2) + pow(y0-car.second,2) - pow(this->ld,2));
        }
...
PurePursuitController::PurePursuitController(double ld, double velocity,
